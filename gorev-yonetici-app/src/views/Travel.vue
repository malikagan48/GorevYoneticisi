        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                <div class="text-center">
                    <ion-icon :icon="airplane" size="large" class="text-green-600"></ion-icon>
                </div>
                <div class="text-center">
                    <ion-card-title>Yolculuk Görevleri</ion-card-title>
                </div>
            </div>
            <div>
                <ion-list>
...
export default defineComponent({
    ...
    setup () {
        ...
        const state: any=reactive({
            ...
            yolculukGorev:computed(()=>{
                return store.getters.kategoriyeGoreGorevGetir('Yolculuk');
            }),
            gelen:computed(()=>{
                return store.getters.gelen(state.yolculukGorev);
            }),
            bugun:computed(()=>{
                return store.getters.bugun(state.yolculukGorev);
            }),
            yapilan:computed(()=>{
                return store.getters.yapilan(state.yolculukGorev);
            }),
            gecen:computed(()=>{
                return store.getters.gecen(state.yolculukGorev);
            }),
        })
        ...
        return {ellipsisVertical,airplane,trash,state,gorevTamamla,gorevTamamlama,gorevSil}
    }
})
</script>
