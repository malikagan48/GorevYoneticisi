        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                <div class="text-center">
                    <ion-icon :icon="cardOutline" size="large" class="text-yellow-400"></ion-icon>
                </div>
                <div class="text-center">
                    <ion-card-title>Alışveriş Görevleri</ion-card-title>
                </div>
            </div>
            <div>
                <ion-list>
...
export default defineComponent({
    ...
    setup () {
        ...
        const state: any=reactive({
            ...
            alisverisGorev:computed(()=>{
                return store.getters.kategoriyeGoreGorevGetir('Alışveriş');
            }),
            gelen:computed(()=>{
                return store.getters.gelen(state.alisverisGorev);
            }),
            bugun:computed(()=>{
                return store.getters.bugun(state.alisverisGorev);
            }),
            yapilan:computed(()=>{
                return store.getters.yapilan(state.alisverisGorev);
            }),
            gecen:computed(()=>{
                return store.getters.gecen(state.alisverisGorev);
            }),
        })
        ...
        return {ellipsisVertical,cardOutline,trash,state,gorevTamamla,gorevTamamlama,gorevSil}
    }
})
</script>
