        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                <div class="text-center">
                    <ion-icon :icon="clipboard" size="large" class="text-blue-500"></ion-icon>
                </div>
                <div class="text-center">
                    <ion-card-title>Bütün Görevler</ion-card-title>
                </div>
            </div>
            <div>
                <ion-list>
...
export default defineComponent({
    ...
    setup () {
        ...
        const state=reactive({
            ...
            gelen:computed(()=>{
                return store.getters.gelen(store.state.gorevler);
            }),
            bugun:computed(()=>{
                return store.getters.bugun(store.state.gorevler);
            }),
            yapilan:computed(()=>{
                return store.getters.yapilan(store.state.gorevler);
            }),
            gecen:computed(()=>{
                return store.getters.gecen(store.state.gorevler);
            }),
        })
        ...
        return {ellipsisVertical,clipboard,trash,state,gorevTamamla,gorevTamamlama,gorevSil}
    }
})
</script>
